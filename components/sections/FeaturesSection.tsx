          ...
          <div>
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-3">
              {[
                ...
                <div key={label} className="rounded-lg border border-gold-soft bg-white p-5 text-center shadow-gold">
        ...
        <div className="mt-14 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {advantages.map((item, index) => {
            ...
            return (
              <article
                key={item.title}
                className="group rounded-lg border border-gold-soft bg-white p-6 shadow-gold transition hover:border-gold hover:shadow-gold-lg"
              >
                <div className="flex items-start justify-between gap-4">
                  <span className="font-heading text-4xl font-black text-gold-soft">
                    {String(index + 1).padStart(2, "0")}
                  </span>
                  <span className="grid h-12 w-12 place-items-center rounded-lg border border-gold-soft bg-gold/10 text-gold transition group-hover:bg-gold group-hover:text-white">
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  {item.tags.map((tag) => (
                    <span
                      key={tag}
                      className="inline-flex items-center gap-1 rounded-full border border-gold-soft bg-white px-3 py-1 text-xs font-black text-gold-rich"
                    >
